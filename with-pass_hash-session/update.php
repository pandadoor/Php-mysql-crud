...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $age = trim($_POST['age']);
    $password = trim($_POST['password']);  // Get new password (optional)
    ...
    elseif (!is_numeric($age) || $age < 1 || $age > 150) {
        ...
        if (!empty($password)) {
            ...
            if (strlen($password) < 6) {
                ...
                $stmt = $conn->prepare("UPDATE users SET name = ?, email = ?, age = ?, password = ? WHERE id = ?");
                $stmt->bind_param("ssisi", $name, $email, $age, $hashed_password, $id);
            ...
            // If password field is empty, update without changing password
            
            // UPDATE query WITHOUT password field
...
<?php if ($user): ?>
    ...
    <form method="POST" action="update.php">
        ...
        <p>
            ...
            <br><small>(Leave empty to keep current password - Minimum 6 characters)</small>
        </p>
        <p>
